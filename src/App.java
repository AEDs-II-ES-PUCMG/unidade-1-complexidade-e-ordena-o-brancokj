...
public class App {
    ...
    */
    ...
    public static void main(String[] args) {
        ...
        IOrdenador<Pedido> ordenadorValor = new HeapSort<>();
    ...
    static void localizarPedidosPremium() {
        ...
        int buscador = pesquisaBinariaPorPreco(pedidosOrdenadosPorValor, valorCorte);
        for (int i = 0; i < quantPedidos; i++) {
            relatorio.append(produtosPremium[i] + "\n");
        }

        System.out.println(relatorio.toString());
    }

    static int pesquisaBinariaPorPreco(Pedido[] vetor, Double alvo) {
        int inicio = 0;
        int fim = quantPedidos -1;
        while(inicio <= fim){
            int meio = (inicio + fim) / 2;
            if(vetor[meio].valorFinal() == alvo){
                return meio;
            }else if(vetor[meio].valorFinal() < alvo){
                inicio = meio + 1;
            }else{
                fim = meio - 1;
            }
